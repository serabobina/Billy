...
class BlankOBFv2:
    ...
    def _generate_random_name(self, value: str) -> str:
        if value in self._aliases.keys():
            ...
            # Generate new alias
            while(True):
                name = "_0x" + "".join(random.choices(string.hexdigits[:16], k=random.randint(10, 25)))
    ...
    def _insert_dummy_comments(self) -> None:
        ...
        (0x2000, 0x206F),  # Symbols
        (0x2200, 0x22FF),  # Math
        (0x2500, 0x257F),  # Box Drawing
        (0x1F300, 0x1F5FF),  # Emoji & pictographs
        ]
        code = self._code.splitlines()
        for index in range(len(code) - 1, 0, -1):
            if random.randint(1, 10) > 3:
                spaces = 0
                comment = "# " + random.choice(code).strip()
                for i in code[index]:
                    if i != " ":
                        break
                    else:
...
def main() -> None:
    ...
    if not args.output:
        filename_no_extension = str.rsplit(os.path.basename(args.input), ".", 1)[0]
        args.output = os.path.join(
            os.path.dirname(args.input), f"Obfuscated_{filename_no_extension}.py")
    ...
    try:
        ...
        print("Saved file as `%s`" % args.output)
    except Exception:
